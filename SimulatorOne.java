...
public class SimulatorOne {
    ...
    public void printPath(String destinationName) {
        ...
        if (destinationVertex == null)
            ...
            System.out.println("cannot be helped 0");
        else {
    ...
    public void dijkstra(String source) throws NullPointerException {
        ...
        if (start == null) {
            System.out.println("cannot be helped 1");
            state = true;
        }
        if (this.state == false){
        clearAll();
...
public static void main(String[] args) {
    ...
    for (int j = 0; j < numberOfRequests; j++) { 
        ...
        source_output = "client " + source + " " + destination;
        if (source == null) {
            System.out.println("cannot be helped 2");
            break;
        }
        ...
        if (!(truckToDropOff.state == true || truckToPickUp.state == true || truckToHome.state == true)) {
            ...
            g.dijkstra_version1(driverNumber);
            result_v1 = g.printPath(source);

            g.dijkstra_version2(driverNumber);
            result_v2 = g.printPath(source);

            if (!result_v1.equals(result_v2)) {
                System.out.println("multiple solutions cost " + (int) Float.parseFloat(result_v1.split(" - ")[0]));
            } else {
                truckToPickUp.dijkstra(driverNumber);
                truckToPickUp.printPath(source);
            }

            g.dijkstra_version1(source);
            result_v1 = g.printPath(destination);

            g.dijkstra_version2(source);
            result_v2 = g.printPath(destination);
            ...
            g.dijkstra_version1(destination);
            result_v1 = g.printPath(driverNumber);

            g.dijkstra_version2(destination);
            result_v2 = g.printPath(driverNumber);
